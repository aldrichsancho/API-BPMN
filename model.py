...
def create_meeting_notes(path):
    ...
    for index, story_desc in enumerate(meeting_notes):
        ...
        bpmn_filename = f'proses_{index}.bpmn'
        bpmn_path = os.path.join(app.config['UPLOAD_FOLDER'], bpmn_filename)
        with open(bpmn_path, 'w') as f:
                f.write(res)
        bpmn_files.append(bpmn_path)

    if os.path.exists(path):
        os.remove(path)
        
    from zipfile import ZipFile
    zip_path = os.path.join(app.config['UPLOAD_FOLDER'], 'bpmn_files.zip')
    with ZipFile(zip_path, 'w') as zipf:
        for bpmn_file in bpmn_files:
            zipf.write(bpmn_file, os.path.basename(bpmn_file))
...
@app.route('/create_bpmn', methods=['POST'])
def create_bpmn():
    ...
    if file and allowed_file(file.filename):
        ...
        zip_path = create_meeting_notes(file_path)
        return send_file(zip_path, as_attachment=True, download_name='bpmn_files.zip')
    else:
